...
import { fetchSalesOrders, createSalesOrder, fetchCustomers, fetchItems, getItemPrice, SalesOrder, Customer, Item, submitSalesOrder, makeDelivery, makeInvoice, fetchSalesTaxTemplates, TaxTemplate } from '@/lib/api';
...
export default function SalesOrdersPage() {
    ...
    const loadData = async () => {
        const [ordersData, customersData, itemsData, taxesData] = await Promise.all([
            fetchSalesOrders(),
            fetchCustomers(),
            fetchItems(),
    ...
    const handleItemChange = async (itemCode: string) => {
        ...
        try {
            const priceData = await getItemPrice(itemCode, 'selling');
        ...
        <div className="p-8">
            <div className="flex justify-between items-center mb-8">
                ...
                <div className="space-x-4">
                    <Link href="/selling/customers" className="text-blue-600 hover:underline">View Customers</Link>
                            ...
                            <tr key={o.id}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    ...
                                    <div className="text-xs text-gray-400 mt-1">
                                        {o.delivery_status} | {o.billing_status}
                                    </div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 space-x-2">
                                    {o.status === 'Draft' && (
                                        <button onClick={async () => { await submitSalesOrder(o.id); loadData(); }} className="text-blue-600 hover:text-blue-900">Submit</button>
                                    )}
                                    {o.status === 'Submitted' && o.delivery_status !== 'Fully Delivered' && (
                                        <button onClick={async () => { await makeDelivery(o.id); loadData(); }} className="text-green-600 hover:text-green-900">Make Delivery</button>
                                    )}
                                    {o.status === 'Submitted' && o.billing_status !== 'Fully Billed' && (
                                        <button onClick={async () => { await makeInvoice(o.id); loadData(); }} className="text-purple-600 hover:text-purple-900">Make Invoice</button>
                                    )}
                                </td>
                            </tr>
                        ))}
