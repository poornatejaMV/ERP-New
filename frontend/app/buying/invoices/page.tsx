...
import { fetchPurchaseInvoices, PurchaseInvoice, createPurchaseReturn, submitPurchaseReturn } from '@/lib/api';
...
export default function PurchaseInvoicesPage() {
    ...
    const handleCreateReturn = async (id: number) => {
        if (!confirm('Are you sure you want to create a return (Debit Note) for this invoice?')) return;
        
        setLoading(true);
        try {
            const res = await createPurchaseReturn(id);
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {invoices.map((inv: any) => (
                            <tr key={inv.id} className={inv.is_return ? "bg-red-50" : ""}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">Supplier #{inv.supplier_id}</td>
